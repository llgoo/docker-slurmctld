...
if [ -z ${SLURM_BASE_FQDN+x} ]; then echo "SLURM_NODE_NAMES not set" && exit 1; fi
...
sed -i -e "s/###SLURM_NODE_NAMES###/${SLURM_NODE_NAMES}/g" $SLURM_CONF_LOCATION
sed -i -e "s/###SLURM_BASE_FQDN###/${SLURM_BASE_FQDN}/g" $SLURM_CONF_LOCATION
